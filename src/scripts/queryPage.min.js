...
const loadingEl = document.querySelector(".loading"),
  ...
  contentEl = containerEl.querySelector(".markdown-html"),
  menuEl = document.querySelector(".menu"),
  isTouchDevice = "ontouchstart" in document.documentElement,
  pageAudio = new Audio("/assets/audios/page.mp3");
...
async function renderPage() {
  ...
  const path = window.location.pathname,
    paramsObj = getParamsByUrl(currentUrl);
  ...
  "/" !== lastCharacterOfCurrentUrl && (currentUrl += "/");
  const splitPathArray = path.split("/"),
    currentFolder = splitPathArray[splitPathArray.length - 2];
  await renderMarkdownPage(path, {
    md: currentFolder,
    highlight: "about" === currentFolder ? "false" : "true",
  });
}
...
async function renderMarkdownPage(path, paramsObj) {
  let { md: md, align: align, highlight: highlight } = paramsObj;
  if (
    (md && (md = md.replaceAll("/", "")),
    (align = align || "justify"),
...
async function renderContent(
  ...
  (contentEl.innerHTML =
    markdownit
      ...
      .replaceAll(" --&gt;", "</div>")
      .replaceAll("[ ]", '<span class="checkbox">☐</span>')
      .replaceAll("[x]", '<span class="checkbox check">☑︎</span>') +
    contentEl.innerHTML),
    "true" === highlight && hljs.highlightAll(),
    ...
    contentEl.querySelectorAll("img").forEach((img) => {
      const {
          ...
          align: align,
          c: c,
        } = getParamsByUrl(img.src),
        magnification = 1.5;
      s && (img.src = img.src.replace(`s=${s}`, `s=${1.5 * s}`)),
        width && (img.style.width = 1.5 * width + "px"),
        align && (img.closest("p").className = align),
        c && img.classList.add(c);
    });
}
function getParamsByUrl(url = window.location.href) {
  const urlSearch = url.split("?")[1],
    urlSearchParams = new URLSearchParams(urlSearch);
...
function updateSiteTitle(headerText) {
  headerText && (document.title = headerText);
}
  ...
  contentEl.querySelectorAll("a").forEach((a) => {
    ...
    aText[0] + aText[1] === "> " &&
      ...
      (a.parentElement.innerHTML =
        a.parentElement.innerHTML +
        ` / <a href="https://github.com/${author}/${repo}" target="_blank" rel="noreferrer noopener">GitHub Repo</a>`);
  });
}
...
(document.onselectstart = () => !1),
  ...
  (window.onload = async () => {
    await renderPage(),
      isTouchDevice
        ? await endLoading()
        : (activateHoverInteraction(contentEl), await endLoading(0.25)),
      enableScroll(),
      resizeBodyHeight();
  }),
  (window.onscroll = () => hideEl(menuEl)),
  (window.onresize = () => {
    hideEl(menuEl), resizeBodyHeight();
  }),
